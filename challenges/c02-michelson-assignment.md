...
- Our counts match Michelson’s exactly (46, 39, 15), but our mean
  velocity for each distinctness is slightly off.

- Our mean for distinctness 3 is 1.7 km/s higher than his, distinctness
  2 is 1.5 km/s lower, and distinctness 1 is 2 km/s lower.

- Distinctness 3 and 2 have nearly identical means around 299860.
  Distinctness 1 is about 50 km/s lower, but it has only 15
  observations, so that mean is less concrete than the other two.

- Why might your table differ from Michelson’s?

  - All three of Michelson’s means end in 0. A mean of dozens of values
    is unlikely to land exactly on a multiple of 10 by chance, so his
    numbers were most likely rounded to the nearest 10 km/s. If we
    rounded the mean velocities we got from the summary, we would get
    his values exactly. Every raw Velocity value in the dataset is also
    a multiple of 10.
...
- Is Michelson’s estimate of the error (his uncertainty) greater or less
  than the true error?
  - Less than the true error. The true error is about −152 km/s, meaning
    his estimate was too high by 152. His stated uncertainty was only +
    or -51.
- Make a quantitative comparison between Michelson’s uncertainty and his
  error.
  - The true error is about 3 times larger than his uncertainty (152 vs
    51). The modern value, 299792 km/s, sits well outside his range of
    299893 km/s to 299995 km/s.
...
- Both panels are centered on the dotted line at Michelson’s estimate.
  This is by construction, since the simulated data uses the real data’s
  mean and standard deviation.
- The spread of the individual points is about the same in both panels.
  Roughly half the points in each panel fall outside the +/- 51 band.
- In both panels, the grey daily mean line crosses in and out of the
  band rather than staying inside it.

Differences

- There is a difference in where the big swings of the daily mean line
  happen. In the Real panel, the three daily means that sit above the
  upper band line and the one that sits far below the lower band line
  all occur before Jun 16. After Jun 16, no real daily mean rises above
  the band again. In the Simulated panel, the only two daily means above
  the band are late, after Jun 23, and nothing before Jun 23 leaves the
  band at all.
- There is a difference in the size of the swings of the daily mean
  line. The real line has one jump of about 200 km/s between consecutive
  days, from below the band to above it, around Jun 12. The simulated
  line never jumps that far between neighboring days.
- There is a difference in the extreme individual points. The highest
  single point (above 300150) and the lowest single point (near 299710)
  are both in the Real panel. The simulated points stay within roughly
  299760 to 300090.
...
``` r
## velocity by image distinctness
df_q2 %>%
...
``` r
## velocity over time, colored by distinctness
df_q2 %>%
  ggplot(aes(Date, VelocityVacuum, color = Distinctness)) +
  geom_point()
```

![](c02-michelson-assignment_files/figure-gfm/q5-date-1.png)<!-- -->
...
- In the distinctness boxplot, the medians step up slightly from about
  ...
  true value. The three boxes overlap heavily and distinctness 1 has
  only 15 points, so the difference between groups is small and could be
  noise.
- In the date plot, the measurements before Jun 16 include more points
  above 300000 than the measurements after. The distinctness 1 points
  (red) only appear on a few dates in the middle of June, so the lower
